...
#### wsgihandler.py
`wsgihandler.py` file is necessary for WSGI gateways (such as uWSGI) to run your Django application and also required from Django itself. You definitely want to change `ifwm.settings` value in this file to whatever you name your application (e.g. `myproject.settings`).
...
It is good idea to make a **find & replace** within this default settings file as there are some "ifwm" string left such as `ROOT_URLCONF` or `LOCAL_APPS` variables, but not only.
